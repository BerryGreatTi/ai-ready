...
def _make_tool(platform: MagicMock | None = None) -> OpenClawTool:
    if platform is None:
        platform = _make_platform()
    i18n = MagicMock()
    i18n.get.side_effect = lambda key, **kwargs: key
    return OpenClawTool(platform=platform, i18n=i18n)


class TestOpenClawToolName:
    def test_get_name(self):
        tool = _make_tool()
        assert tool.get_name() == "OpenClaw"
...
class TestOpenClawVerifyCommands:
    def test_verify_commands(self):
        ...
        assert "openclaw --version" in cmds
        assert "openclaw doctor" in cmds
        assert "openclaw gateway status" in cmds
