...
# AIReady - OpenClaw Installer for Linux
# =======================================
...
msg() {
    ...
    case "${LANG_ID}_${key}" in
        ...
        ko_distro_unknown)  echo "알 수 없는 배포판. 수동으로 Node.js를 설치하세요." ;;
        ...
        en_distro_unknown)  echo "Unknown distro. Install Node.js manually." ;;
        *) echo "$key" ;;
    esac
}
...
if [[ "$(uname -s)" != "Linux" ]]; then
    step_fail "This script is for Linux only"
    exit 1
fi
...
step_ok "Linux / ${DISTRO} (pkg: ${PKG_MANAGER})"
